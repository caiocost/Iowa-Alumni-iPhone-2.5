...
//Rare exceptions to indicate a bug in the _uiowaalumni code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_uiowaalumni = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_uiowaalumni]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.uiowaalumni.module"
