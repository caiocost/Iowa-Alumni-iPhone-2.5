...
function Post(item) {//<----
	...
	var startTime = formatDate(startDate); //startDate.getHours() + ":" + (startDate.getMinutes() < 10 ? '0' : '') + startDate.getMinutes() + (item.meridiem == undefined ? '' : item.meridiem);
    ...
    this.snl = startTime + (endTime != startTime ? " - " + endTime : '');//
...
function convertUTCDateToLocalDate(date, timeZone) {
    var timeZoneOffset, utc, newDate;
